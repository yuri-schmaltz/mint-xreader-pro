...
G_DEFINE_TYPE_WITH_CODE(EvAnnotationTextMarkup, ev_annotation_text_markup,
                        EV_TYPE_ANNOTATION, {
                          G_IMPLEMENT_INTERFACE(
                              EV_TYPE_ANNOTATION_MARKUP,
...
G_DEFINE_TYPE_WITH_CODE(EvAnnotationFreeText, ev_annotation_free_text,
                        EV_TYPE_ANNOTATION, {
                          G_IMPLEMENT_INTERFACE(
                              EV_TYPE_ANNOTATION_MARKUP,
...
static void ev_annotation_class_init(EvAnnotationClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "page", "Page", "The page wehere the annotation is", EV_TYPE_PAGE,
   ...
   */
  g_object_class_install_property(
      g_object_class, PROP_ANNOT_AREA,
      g_param_spec_boxed(
          "area", "Area", "The area of the page where the annotation is placed",
          EV_TYPE_RECTANGLE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void ev_annotation_markup_class_install_properties(GObjectClass *klass) {
  ...
  g_object_class_override_property(klass, PROP_MARKUP_CAN_HAVE_POPUP,
                                   "can-have-popup");
...
static void ev_annotation_text_class_init(EvAnnotationTextClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("is-open", "IsOpen",
...
ev_annotation_text_markup_class_init(EvAnnotationTextMarkupClass *class) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_TEXT_MARKUP_TYPE,
      g_param_spec_enum("type", "Type", "The text markup annotation type",
...
static void ev_annotation_text_markup_markup_iface_init(
    EvAnnotationMarkupInterface *iface) {}

EvAnnotation *ev_annotation_text_markup_highlight_new(EvPage *page) {
  EvAnnotation *annot = EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_TEXT_MARKUP, "page", page, "type",
                   EV_ANNOTATION_TEXT_MARKUP_HIGHLIGHT, NULL));
  return annot;
}
...
EvAnnotation *ev_annotation_text_markup_strike_out_new(EvPage *page) {
  EvAnnotation *annot = EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_TEXT_MARKUP, "page", page, "type",
                   EV_ANNOTATION_TEXT_MARKUP_STRIKE_OUT, NULL));
  return annot;
}
...
EvAnnotation *ev_annotation_text_markup_underline_new(EvPage *page) {
  EvAnnotation *annot = EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_TEXT_MARKUP, "page", page, "type",
                   EV_ANNOTATION_TEXT_MARKUP_UNDERLINE, NULL));
  return annot;
}
...
EvAnnotation *ev_annotation_text_markup_squiggly_new(EvPage *page) {
  EvAnnotation *annot = EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_TEXT_MARKUP, "page", page, "type",
                   EV_ANNOTATION_TEXT_MARKUP_SQUIGGLY, NULL));
  return annot;
}

EvAnnotationTextMarkupType
ev_annotation_text_markup_get_markup_type(EvAnnotationTextMarkup *annot) {
  g_return_val_if_fail(EV_IS_ANNOTATION_TEXT_MARKUP(annot),
                       EV_ANNOTATION_TEXT_MARKUP_HIGHLIGHT);

  return annot->type;
}

gboolean ev_annotation_text_markup_set_markup_type(
    EvAnnotationTextMarkup *annot, EvAnnotationTextMarkupType markup_type) {
...
static void ev_annotation_free_text_init(EvAnnotationFreeText *annot) {
...
EvAnnotation *ev_annotation_free_text_new(EvPage *page) {
  return EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_FREE_TEXT, "page", page, NULL));
}

/* EvAnnotationInk */
static void ev_annotation_ink_finalize(GObject *object) {
  EvAnnotationInk *annot = EV_ANNOTATION_INK(object);

  if (annot->ink_list) {
    g_list_free_full(annot->ink_list, (GDestroyNotify)g_array_unref);
    annot->ink_list = NULL;
  }
...
static void ev_annotation_ink_init(EvAnnotationInk *annot) {
  EV_ANNOTATION(annot)->type = EV_ANNOTATION_TYPE_INK;
}

static void ev_annotation_ink_class_init(EvAnnotationInkClass *klass) {
  GObjectClass *g_object_class = G_OBJECT_CLASS(klass);
...
EvAnnotation *ev_annotation_ink_new(EvPage *page) {
  return EV_ANNOTATION(
      g_object_new(EV_TYPE_ANNOTATION_INK, "page", page, NULL));
}

GList *ev_annotation_ink_get_ink_list(EvAnnotationInk *ink) {
  g_return_val_if_fail(EV_IS_ANNOTATION_INK(ink), NULL);

  return ink->ink_list;
}

gboolean ev_annotation_ink_set_ink_list(EvAnnotationInk *ink, GList *ink_list) {
  g_return_val_if_fail(EV_IS_ANNOTATION_INK(ink), FALSE);

  if (ink->ink_list == ink_list)
    return FALSE;

  if (ink->ink_list)
    g_list_free_full(ink->ink_list, (GDestroyNotify)g_array_unref);

  ink->ink_list = ink_list;
