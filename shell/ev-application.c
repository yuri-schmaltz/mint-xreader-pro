...
static void
_ev_application_open_uri_at_dest(EvApplication *application, const gchar *uri,
                                 GdkScreen *screen, EvLinkDest *dest,
                                 EvWindowRunMode mode,
                                 const gchar *search_string, guint timestamp);
static void
ev_application_open_uri_in_window(EvApplication *application, const char *uri,
                                  EvWindow *ev_window, GdkScreen *screen,
...
static void ev_application_register_uri(EvApplication *application,
  ...
  g_dbus_connection_call(
      ...
      XREADER_DAEMON_SERVICE, XREADER_DAEMON_OBJECT_PATH,
      XREADER_DAEMON_INTERFACE, "RegisterDocument", g_variant_new("(s)", uri),
      G_VARIANT_TYPE("(s)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL,
      on_register_uri_cb, data);
...
static void ev_application_unregister_uri(EvApplication *application,
  ...
  value = g_dbus_connection_call_sync(
      g_application_get_dbus_connection(G_APPLICATION(application)),
      XREADER_DAEMON_SERVICE, XREADER_DAEMON_OBJECT_PATH,
      XREADER_DAEMON_INTERFACE, "UnregisterDocument", g_variant_new("(s)", uri),
...
_ev_application_open_uri_at_dest(EvApplication *application, const gchar *uri,
  ...
  if (!ev_window) {
    ...
    if (windows)
      ev_window = EV_WINDOW(windows->data);
  }
...
void ev_application_open_uri_at_dest(EvApplication *application,
  ...
  if (application->uri && strcmp(application->uri, uri) != 0) {
    /* Open in a new tab of the existing window instead of spanning a new
     * process */
...
#ifdef ENABLE_DBUS
static gboolean handle_get_window_list_cb(EvXreaderApplication *object,
...
static gboolean handle_reload_cb(EvXreaderApplication *object,
                                 GDBusMethodInvocation *invocation,
  ...
  if (display != NULL && screen_number >= 0 &&
...
static void parse_mimetypes(void) {
...
static void ev_application_init(EvApplication *ev_application) {
  ev_application->dot_dir =
      g_build_filename(g_get_user_config_dir(), "xreader", NULL);
  ...
  g_object_set(ev_application->scr_saver, "reason",
               _("Running in presentation mode"), NULL);
