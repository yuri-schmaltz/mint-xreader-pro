...
static void copy_job_to_job_info(EvJobRender *job_render,
  ...
  if (job_info->job) {
    g_signal_handlers_disconnect_by_func(
        job_info->job, G_CALLBACK(job_finished_cb), pixbuf_cache);
    ev_job_cancel(job_info->job);
    g_object_unref(job_info->job);
    job_info->job = NULL;
  }
...
static void ev_pixbuf_cache_add_jobs_if_needed(EvPixbufCache *pixbuf_cache,
  ...
  for (i = FIRST_VISIBLE_PREV(pixbuf_cache);
       i < pixbuf_cache->preload_cache_size; i++) {
...
cairo_region_t *
ev_pixbuf_cache_get_selection_region(EvPixbufCache *pixbuf_cache, gint page,
                                     gfloat scale) {
  CacheJobInfo *job_info;
