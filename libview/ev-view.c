...
#define EV_VIEW_CLASS(klass)                                                   \
  (G_TYPE_CHECK_CLASS_CAST((klass), EV_TYPE_VIEW, EvViewClass))
#define EV_IS_VIEW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), EV_TYPE_VIEW))
#define EV_VIEW_GET_CLASS(obj)                                                 \
  (G_TYPE_INSTANCE_GET_CLASS((obj), EV_TYPE_VIEW, EvViewClass))
...
static void map_annot_to_window(EvView *view, EvAnnotation *annot,
                                GtkWidget *window);
...
static void highlight_forward_search_results(EvView *view, cairo_t *cr,
                                             int page);
static void focus_annotation(EvView *view, cairo_t *cr, int page,
                             GdkRectangle *clip);
...
static double zoom_for_size_fit_height(gdouble doc_width, gdouble doc_height,
                                       int target_width, int target_height);
static double zoom_for_size_best_fit(gdouble doc_width, gdouble doc_height,
                                     int target_width, int target_height);
static gboolean ev_view_can_zoom(EvView *view, gdouble factor);
...
#define PAGE_CACHE_NUMBER 10
  ev_page_cache_set_page_range(
      view->page_cache, MAX(view->start_page - PAGE_CACHE_NUMBER, 0),
      MIN(view->end_page + PAGE_CACHE_NUMBER,
          ev_document_get_n_pages(view->document) - 1));
  ev_pixbuf_cache_set_page_range(
      view->pixbuf_cache, MAX(view->start_page - PAGE_CACHE_NUMBER, 0),
      MIN(view->end_page + PAGE_CACHE_NUMBER,
          ev_document_get_n_pages(view->document) - 1),
...
static void add_scroll_binding_keypad(GtkBindingSet *binding_set, guint keyval,
  ...
  gtk_binding_entry_add_signal(binding_set, keyval, modifiers,
                               "binding_activated", 2, GTK_TYPE_SCROLL_TYPE,
                               scroll, G_TYPE_BOOLEAN, horizontal);
...
static void get_page_y_offset(EvView *view, int page, int *y_offset) {
  ...
  if (view->dual_page) {
    ...
    offset +=
        ((page + view->dual_even_left) / 2 + 1) * view->spacing +
...
gboolean ev_view_get_page_extents(EvView *view, gint page,
  ...
  if (view->continuous) {
    ...
    if (view->dual_page) {
      gboolean right_page;
      right_page = (gtk_widget_get_direction(widget) == GTK_TEXT_DIR_LTR &&
                    page % 2 == view->dual_even_left) ||
...
void _ev_view_transform_view_point_to_doc_point(
  ...
  *doc_point_x =
      (double)(view_point->x - page_area->x - border->left) / view->scale;
  *doc_point_y =
      (double)(view_point->y - page_area->y - border->right) / view->scale;
}
...
void _ev_view_transform_view_rect_to_doc_rect(EvView *view,
                                              ...
                                              EvRectangle *doc_rect) {
  doc_rect->x1 =
      (double)(view_rect->x - page_area->x - border->left) / view->scale;
  doc_rect->y1 =
      (double)(view_rect->y - page_area->y - border->right) / view->scale;
...
static gboolean location_in_selected_text(EvView *view, gdouble x, gdouble y) {
  ...
  for (l = view->selection_info.selections; l != NULL; l = l->next) {
    ...
    if (selection->covered_region &&
        cairo_region_contains_point(selection->covered_region, x_offset,
                                    y_offset))
      return TRUE;
  }
...
static void goto_fit_dest(EvView *view, EvLinkDest *dest) {
  ...
  zoom = zoom_for_size_best_fit(doc_width, doc_height, allocation.width,
                                allocation.height);
...
static char *tip_from_link(EvView *view, EvLink *link) {
  ...
  case EV_LINK_ACTION_TYPE_EXTERNAL_URI:
    ...
    break;
  case EV_LINK_ACTION_TYPE_LAUNCH:
    msg = g_strdup_printf(_("Launch %s"), ev_link_action_get_filename(action));
    break;
  case EV_LINK_ACTION_TYPE_NAMED:
...
void ev_view_set_focused_element(EvView *view, EvMapping *element_mapping,
                                 gint page) {
  GdkRectangle view_rect;
...
static void ev_view_remove_all_form_fields(EvView *view) {
  gtk_container_foreach(GTK_CONTAINER(view),
                        (GtkCallback)destroy_child_if_form_widget, NULL);
}
...
static GtkWidget *ev_view_form_field_button_create_widget(EvView *view,
  ...
  /* We need to do this focus grab prior to setting the focused element for
   * accessibility */
...
static gboolean ev_view_form_field_text_button_pressed(GtkWidget *widget,
                                                       GdkEventButton *event,
                                                       gpointer data) {
...
void ev_view_focus_form_field(EvView *view, EvFormField *field) {
...
static void ev_view_remove_window_child_for_annot(EvView *view, guint page,
                                                  EvAnnotation *annot) {
  GList *children = view->window_children;
  ...
  while (children) {
    ...
    if (ev_annotation_equal(wannot, annot)) {
      ...
      view->window_children =
          g_list_delete_link(view->window_children, children);
      break;
    }
    children = children->next;
  }
...
static void ev_view_handle_annotation(EvView *view, EvAnnotation *annot,
  ...
  if (EV_IS_ANNOTATION_MARKUP(annot)) {
    ...
    window = get_window_for_annot(view, annot);
    if (!window &&
        ev_annotation_markup_can_have_popup(EV_ANNOTATION_MARKUP(annot))) {
      EvRectangle popup_rect;
      GtkWindow *parent;
      EvMappingList *annots;
      EvMapping *mapping;

      annots = ev_page_cache_get_annot_mapping(
          view->page_cache, ev_annotation_get_page_index(annot));
...
static void ev_view_create_annotation(EvView *view) {
  ...
  _ev_view_transform_view_point_to_doc_point(
      view, &view->adding_annot_info.start, &page_area, &border, &start.x,
      &start.y);
  _ev_view_transform_view_point_to_doc_point(
      view, &view->adding_annot_info.stop, &page_area, &border, &end.x, &end.y);
  ...
  case EV_ANNOTATION_TYPE_INK: {
    ...
    doc_rect.x2 = end.x;
    doc_rect.y2 = end.y;
    annot = ev_annotation_ink_new(page);

    array = g_array_new(FALSE, TRUE, sizeof(EvPoint));
    g_array_append_val(array, start);
    view->adding_annot_info.path = g_list_append(NULL, array);
  } break;
  default:
    g_assert_not_reached();
  }
  ...
  /* Re-fetch area as eg. adding Text Markup annots updates area for its
   * bounding box */
...
void ev_view_focus_annotation(EvView *view, EvMapping *annot_mapping) {
  ...
  _ev_view_transform_doc_rect_to_view_rect(view, page, &annot_mapping->area,
                                           &view_rect);
  ensure_rectangle_is_visible(view, &view_rect);
  gtk_widget_queue_draw(GTK_WIDGET(view));
}
...
static gboolean ev_view_scroll_event(GtkWidget *widget, GdkEventScroll *event) {
  ...
  if (state == 0 && !view->continuous && (fit_width || fit_height)) {
    ...
    case GDK_SCROLL_SMOOTH: {
      gdouble decrement;
      if ((fit_width && fit_height) || ((fit_height && event->delta_x == 0.0) ||
  ...
  if (event->direction == GDK_SCROLL_SMOOTH && event->delta_x != 0.0 &&
      event->delta_y != 0.0) {
    gdouble abs_x, abs_y;
...
static gboolean ev_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  for (i = view->start_page; i >= 0 && i <= view->end_page; i++) {
    ...
    if (page_ready && view->adding_annot_info.adding_annot &&
        view->adding_annot_info.type == EV_ANNOTATION_TYPE_INK &&
        view->adding_annot_info.path && view->current_page == i) {
      GList *l;

      cairo_save(cr);
      cairo_set_source_rgb(cr, 0., 1., 1.); /* Cyan preview */
      cairo_set_line_width(cr, 2.0);

      for (l = view->adding_annot_info.path; l; l = g_list_next(l)) {
        GArray *path = (GArray *)l->data;
        guint j;

        for (j = 0; j < path->len; j++) {
          EvPoint *p = &g_array_index(path, EvPoint, j);
          GdkPoint vp;

          _ev_view_transform_doc_point_to_view_point(view, i, p, &vp);
          if (j == 0)
            cairo_move_to(cr, vp.x - view->scroll_x, vp.y - view->scroll_y);
          else
            cairo_line_to(cr, vp.x - view->scroll_x, vp.y - view->scroll_y);
        }
      }
      cairo_stroke(cr);
      cairo_restore(cr);
  ...
  case 1: {
    ...
    if (EV_IS_SELECTION(view->document) && view->selection_info.selections) {
      ...
      ev_view_handle_form_field(view, field);
                                 ...
                                 event->y + view->scroll_y) &&
               (image =
                    ev_view_get_image_at_location(view, event->x, event->y))) {
  ...
  case 1:
    ...
    if (view->adding_annot_info.adding_annot) {
      ...
      _ev_view_transform_view_point_to_doc_point(
          ...
          &start.y);
      _ev_view_transform_view_point_to_doc_point(
          view, &view->adding_annot_info.stop, &page_area, &border, &end.x,
          &end.y);
      ...
      case EV_ANNOTATION_TYPE_INK: {
        ...
        rect.x1 = MIN(current_area.x1, end.x);
        rect.y1 = MIN(current_area.y1, end.y);
        rect.x2 = MAX(current_area.x2, end.x);
        rect.y2 = MAX(current_area.y2, end.y);

        array = (GArray *)g_list_last(view->adding_annot_info.path)->data;
        doc_point.x = end.x;
        doc_point.y = end.y;
        g_array_append_val(array, doc_point);
      } break;
      default:
        g_assert_not_reached();
      }
      ...
      if (ev_annotation_set_area(view->adding_annot_info.annot, &rect)) {
        ev_document_annotations_save_annotation(
            EV_DOCUMENT_ANNOTATIONS(view->document),
            view->adding_annot_info.annot, EV_ANNOTATIONS_SAVE_AREA);
      }
      ev_document_doc_mutex_unlock();
  ...
  if (view->adding_annot_info.adding_annot) {
    ...
    if (EV_IS_ANNOTATION_MARKUP(view->adding_annot_info.annot)) {
      ...
      if (area.x1 == 0 && area.y1 == 0 && area.x2 == 0 && area.y2 == 0) {
        ...
        if (ev_annotation_markup_set_rectangle(
                EV_ANNOTATION_MARKUP(view->adding_annot_info.annot),
                &popup_rect)) {
          ev_document_doc_mutex_lock();
          ev_document_annotations_save_annotation(
              EV_DOCUMENT_ANNOTATIONS(view->document),
              view->adding_annot_info.annot, EV_ANNOTATIONS_SAVE_POPUP_RECT);
          ev_document_doc_mutex_unlock();
        }

        if (EV_IS_ANNOTATION_INK(view->adding_annot_info.annot)) {
          ev_annotation_ink_set_ink_list(
              EV_ANNOTATION_INK(view->adding_annot_info.annot),
              view->adding_annot_info.path);
          view->adding_annot_info.path = NULL;

          ev_document_doc_mutex_lock();
          ev_document_annotations_save_annotation(
              EV_DOCUMENT_ANNOTATIONS(view->document),
              view->adding_annot_info.annot, EV_ANNOTATIONS_SAVE_ALL);
          ev_document_doc_mutex_unlock();
        }
        ...
        if (window == NULL) {
          parent = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(view)));
          window = ev_view_create_annotation_window(
              view, view->adding_annot_info.annot, parent);
        }
        /* Show the annot window the first time for text annotations */
        if (view->adding_annot_info.type == EV_ANNOTATION_TYPE_TEXT ||
            view->adding_annot_info.type == EV_ANNOTATION_TYPE_FREE_TEXT)
          ev_view_annotation_show_popup_window(view, window);
      }
    }
    ...
    view->adding_annot_info.stop.y = event->y + view->scroll_y;
    if (annot_added)
      g_signal_emit(view, signals[SIGNAL_ANNOT_ADDED], 0,
                    view->adding_annot_info.annot);

    if (view->adding_annot_info.path) {
      g_list_free_full(view->adding_annot_info.path,
...
static void focus_annotation(EvView *view, cairo_t *cr, gint page,
                             GdkRectangle *clip) {
  GtkWidget *widget = GTK_WIDGET(view);
  GdkRectangle rect;
  EvMapping *mapping = view->focus_annotation;
  ...
  gtk_render_focus(gtk_widget_get_style_context(widget), cr,
                   rect.x - view->scroll_x, rect.y - view->scroll_y,
...
static gboolean show_loading_window_cb(EvView *view) {
...
static void show_loading_window(EvView *view) {
  ...
  if (!view->loading_timeout) {
    view->loading_timeout = g_timeout_add_full(
        G_PRIORITY_LOW, 0.5, (GSourceFunc)show_loading_window_cb, view, NULL);
  }
}
...
static void draw_surface(cairo_t *cr, cairo_surface_t *surface, gint x, gint y,
  ...
  if (width != target_width || height != target_height) {
    ...
    cairo_scale(cr, (gdouble)target_width / width * device_scale_x,
...
static gint ev_view_mapping_compare(const EvMapping *a, const EvMapping *b,
                                    gpointer user_data) {
...
static GList *ev_view_get_sorted_mapping_list(EvView *view,
  ...
  mapping_list = g_list_sort_with_data(
      g_list_reverse(mapping_list), (GCompareDataFunc)ev_view_mapping_compare,
...
static void schedule_child_focus_in_idle(EvView *view,
  ...
  view->child_focus_idle_id = g_idle_add(direction == GTK_DIR_TAB_FORWARD
                                             ? child_focus_forward_idle_cb
...
static gboolean ev_view_focus_next(EvView *view, GtkDirectionType direction) {
  ...
  if (view->focused_element) {
    ...
    elements = ev_view_get_sorted_mapping_list(view, direction,
    ...
    elements =
        ev_view_get_sorted_mapping_list(view, direction, view->current_page);
   ...
   * a focused element. This prevents the view from jumping to the first/last
   * page when there are not focusable elements.
   */
...
static void ev_view_activate(EvView *view) {
  ...
  if (EV_IS_DOCUMENT_FORMS(view->document) &&
    ...
    view->key_binding_handled = ev_view_activate_form_field(
        view, EV_FORM_FIELD(view->focused_element->data));
    return;
  }

  if (EV_IS_DOCUMENT_LINKS(view->document) &&
      EV_IS_LINK(view->focused_element->data)) {
    view->key_binding_handled =
        ev_view_activate_link(view, EV_LINK(view->focused_element->data));
    return;
  }
}
...
static void ev_view_class_init(EvViewClass *class) {
  ...
  signals[SIGNAL_BINDING_ACTIVATED] =
      g_signal_new("binding_activated", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EvViewClass, binding_activated), NULL, NULL,
                   ev_view_marshal_VOID__ENUM_BOOLEAN, G_TYPE_NONE, 2,
  ...
  signals[SIGNAL_ACTIVATE] =
      g_signal_new("activate", G_OBJECT_CLASS_TYPE(object_class),
                   ...
                   G_STRUCT_OFFSET(EvViewClass, activate), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
  ...
  add_scroll_binding_keypad(binding_set, GDK_KEY_Up, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_DOWN, FALSE);
  add_scroll_binding_keypad(binding_set, GDK_KEY_Down, GDK_MOD1_MASK,
                            GTK_SCROLL_STEP_UP, FALSE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_H, 0, "binding_activated",
                               2, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_STEP_BACKWARD, G_TYPE_BOOLEAN, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_J, 0, "binding_activated",
                               2, GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_STEP_FORWARD,
                               G_TYPE_BOOLEAN, FALSE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_K, 0, "binding_activated",
                               2, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_STEP_BACKWARD, G_TYPE_BOOLEAN, FALSE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_L, 0, "binding_activated",
                               2, GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_STEP_FORWARD,
                               G_TYPE_BOOLEAN, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_space, 0, "activate", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Space, 0, "activate", 0);
...
static void ev_view_dual_page_changed_cb(EvDocumentModel *model,
                                         GParamSpec *pspec, EvView *view) {
  gboolean dual_page = ev_document_model_get_dual_page(model);

  view->dual_page =
      dual_page || ev_document_model_get_dual_page_odd_pages_left(model);
  view->pending_scroll = SCROLL_TO_PAGE_POSITION;
...
static void update_rtl_navigation(gboolean rtl) {
  GtkBindingSet *binding_set;
  binding_set =
      gtk_binding_set_by_class(GTK_WIDGET_CLASS(ev_view_parent_class));
  if (!rtl) {
...
static void ev_view_direction_changed_cb(EvDocumentModel *model,
                                         GParamSpec *pspec, EvView *view) {
...
void ev_view_set_model(EvView *view, EvDocumentModel *model) {
  ...
  g_signal_connect(view->model, "notify::dual-page",
                   G_CALLBACK(ev_view_dual_page_changed_cb), view);
  ...
  g_signal_connect(view->model, "notify::rtl",
                   G_CALLBACK(ev_view_direction_changed_cb), view);
...
static void ev_view_zoom_for_size_continuous_and_dual_page(EvView *view,
  ...
  if (view->sizing_mode == EV_SIZING_FIT_WIDTH) {
    scale =
        zoom_for_size_fit_width(doc_width, doc_height, width - sb_size, height);
  } else if (view->sizing_mode == EV_SIZING_BEST_FIT)
    scale =
        zoom_for_size_best_fit(doc_width, doc_height, width - sb_size, height);
  else
    g_assert_not_reached();
 ...
 */
static GList *compute_new_selection_rect(EvView *view, GdkPoint *start,
                                         GdkPoint *stop) {
  GdkRectangle view_rect;
  int n_pages, i;
  GList *list = NULL;
...
static GList *compute_new_selection_text(EvView *view, EvSelectionStyle style,
                                         GdkPoint *start, GdkPoint *stop) {
...
void ev_view_disconnect_handlers(EvView *view) {
  ...
  g_signal_handlers_disconnect_by_func(
      view->model, G_CALLBACK(ev_view_dual_page_changed_cb), view);
  g_signal_handlers_disconnect_by_func(
