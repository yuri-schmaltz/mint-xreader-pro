...
static void handle_message(const ddjvu_message_t *msg, GError **error) {
  ...
  case DDJVU_ERROR: {
    ...
    if (msg->m_error.filename) {
      ...
      error_str = g_strdup_printf("DjvuLibre error: %s", msg->m_error.message);
    }
...
static gboolean djvu_document_load(EvDocument *document, const char *uri,
  ...
  if (!filename) {
    g_set_error_literal(error, EV_DOCUMENT_ERROR, EV_DOCUMENT_ERROR_INVALID,
                        _("DjVu backend only supports local files"));
    return FALSE;
  }
...
static void djvu_document_get_page_size(EvDocument *document, EvPage *page,
  ...
  document_get_page_size(djvu_document, page->index, width, height, NULL);
}
...
static cairo_surface_t *djvu_document_render(EvDocument *document,
  ...
  document_get_page_size(djvu_document, rc->page->index, &page_width,
                         &page_height, NULL);
...
static void djvu_convert_to_doc_rect(EvRectangle *dest, EvRectangle *source,
                                     gdouble height, gdouble dpi) {
...
static GList *djvu_selection_get_selection_rects(DjvuDocument *djvu_document,
                                                 gint page, EvRectangle *points,
                                                 gdouble height, gdouble dpi) {
  miniexp_t page_text;
  EvRectangle rectangle;
  GList *rects = NULL;
...
static cairo_region_t *djvu_get_selection_region(DjvuDocument *djvu_document,
                                                 gint page, gdouble scale,
                                                 EvRectangle *points) {
  double height, dpi;
  GList *rects = NULL, *l;
...
djvu_selection_get_selection_region(EvSelection *selection, EvRenderContext *rc,
                                    EvSelectionStyle style,
                                    EvRectangle *points) {
  DjvuDocument *djvu_document = DJVU_DOCUMENT(selection);

  return djvu_get_selection_region(djvu_document, rc->page->index, rc->scale,
                                   points);
}
...
static void djvu_document_init(DjvuDocument *djvu_document) {
  ...
  djvu_document->d_context = ddjvu_context_create("Xreader Pro");
...
djvu_document_links_get_links(EvDocumentLinks *document_links, EvPage *page) {
  gdouble dpi;

  document_get_page_size(DJVU_DOCUMENT(document_links), page->index, NULL, NULL,
                         &dpi);
