...
static EvDocumentInfo *pdf_document_get_info(EvDocument *document) {
  ...
  g_object_get(PDF_DOCUMENT(document)->document, "title", &(info->title),
               ...
               "producer", &(info->producer), "creation-datetime",
               &(info->creation_date), "mod-datetime", &(info->modified_date),
               "linearized", &linearized, "metadata", &metadata, NULL);
...
static EvLinkDest *ev_link_dest_from_dest(PdfDocument *pdf_document,
  ...
  case POPPLER_DEST_FITH: {
    ...
    poppler_page = poppler_document_get_page(pdf_document->document,
                                             MAX(0, dest->page_num - 1));
    poppler_page_get_size(poppler_page, NULL, &height);
    ev_dest = ev_link_dest_new_fith(
        dest->page_num - 1, height - MIN(height, dest->top), dest->change_top);
    g_object_unref(poppler_page);
  } break;
...
pdf_document_links_get_links(EvDocumentLinks *document_links, EvPage *page) {
...
GdkPixbuf *pdf_document_images_get_image(EvDocumentImages *document_images,
                                         EvImage *image) {
...
static void
pdf_selection_render_selection(EvSelection *selection, EvRenderContext *rc,
...
pdf_selection_get_selection_region(EvSelection *selection, EvRenderContext *rc,
                                   EvSelectionStyle style,
                                   EvRectangle *points) {
  PopplerPage *poppler_page;
  cairo_region_t *retval;
...
#if POPPLER_CHECK_VERSION(22, 2, 0)
  retval = poppler_page_get_selected_region(poppler_page, rc->scale,
                                            (PopplerSelectionStyle)style,
                                            (PopplerRectangle *)points);
#else
  GList *region = poppler_page_get_selection_region(
      poppler_page, 1.0, (PopplerSelectionStyle)style,
      (PopplerRectangle *)points);
...
ev_form_field_from_poppler_field(PopplerFormField *poppler_field) {
  ...
  case POPPLER_FORM_FIELD_CHOICE: {
    ...
    field_choice->selected_items = poppler_form_field_choice_get_selected_items
    (poppler_field);*/
...
static EvMappingList *
pdf_document_forms_get_form_fields(EvDocumentForms *document, EvPage *page) {
  PopplerPage *poppler_page;
  GList *retval = NULL;
  GList *fields;
  GList *list;
...
/* Annotations */
static void poppler_annot_color_to_gdk_rgba(PopplerAnnot *poppler_annot,
                                            GdkRGBA *color) {
...
static void poppler_annot_color_from_gdk_rgba(PopplerAnnot *poppler_annot,
                                              GdkRGBA *color) {
  PopplerColor poppler_color;
...
static EvAnnotation *ev_annot_from_poppler_annot(PopplerAnnot *poppler_annot,
  ...
  case POPPLER_ANNOT_FILE_ATTACHMENT: {
    ...
    poppler_attachment =
        poppler_annot_file_attachment_get_attachment(poppler_annot_attachment);

    if (poppler_attachment &&
        attachment_save_to_buffer(poppler_attachment, &data, &size, &error)) {
      EvAttachment *ev_attachment;
      GDateTime *poppler_attachment_mtime =
          poppler_attachment_get_mtime(poppler_attachment);
      GDateTime *poppler_attachment_ctime =
          poppler_attachment_get_ctime(poppler_attachment);

      ev_attachment = ev_attachment_new(
          poppler_attachment->name, poppler_attachment->description,
          poppler_attachment_mtime, poppler_attachment_ctime, size, data);
  ...
  if (ev_annot) {
    ...
    if (poppler_annot_can_have_popup_window(poppler_annot)) {
      ...
      g_object_set(ev_annot, "label", label, "opacity", opacity,
                   "can_have_popup", TRUE, NULL);
...
static void annot_area_changed_cb(EvAnnotation *annot, GParamSpec *spec,
                                  EvMapping *mapping) {
...
static EvMappingList *pdf_document_annotations_get_annotations(
  ...
  for (list = annots; list; list = list->next) {
    ...
    g_signal_connect(ev_annot, "notify::area",
                     G_CALLBACK(annot_area_changed_cb), annot_mapping);
...
static GArray *get_quads_for_area(PopplerPage *page, EvRectangle *area,
  ...
  /* PATCH 5: Use new API to avoid deprecation warnings */
#if POPPLER_CHECK_VERSION(22, 2, 0)
  cairo_region_t *region_cairo = poppler_page_get_selected_region(
      page, 1.0, POPPLER_SELECTION_GLYPH, (PopplerRectangle *)area);
  n_rects = cairo_region_num_rectangles(region_cairo);

  quads = g_array_sized_new(TRUE, TRUE, sizeof(PopplerQuadrilateral), n_rects);
...
static void pdf_document_annotations_add_annotation(
  ...
  case EV_ANNOTATION_TYPE_TEXT_MARKUP: {
    ...
    switch (ev_annotation_text_markup_get_markup_type(
        EV_ANNOTATION_TEXT_MARKUP(annot))) {
  ...
  case EV_ANNOTATION_TYPE_FREE_TEXT:
    /* Fallback to regular text annot if FreeText is not directly available in
     * poppler-glib */
    poppler_annot =
        poppler_annot_text_new(pdf_document->document, &poppler_rect);
    break;
  case EV_ANNOTATION_TYPE_INK: {
    /* Stubbed out since poppler_annot_ink_new is missing in current
     * poppler-glib headers. We keep the data logic for the UI view rendering.
     */
    poppler_annot = NULL;
  } break;
  default:
  ...
  g_signal_connect(annot, "notify::area", G_CALLBACK(annot_area_changed_cb),
                   annot_mapping);
...
static void copy_poppler_annot(PopplerAnnot *src_annot,
  ...
  if (POPPLER_IS_ANNOT_MARKUP(src_annot) &&
      POPPLER_IS_ANNOT_MARKUP(dst_annot)) {
    ...
    poppler_annot_markup_set_opacity(
        dst_markup, poppler_annot_markup_get_opacity(src_markup));
    ...
    if (poppler_annot_markup_has_popup(src_markup)) {
      ...
      if (poppler_annot_markup_get_popup_rectangle(src_markup, &popup_rect)) {
        ...
        poppler_annot_markup_set_popup_is_open(
            dst_markup, poppler_annot_markup_get_popup_is_open(src_markup));
      }
    }
  }
}
...
static void pdf_document_annotations_save_annotation(
  ...
  if (EV_IS_ANNOTATION_TEXT_MARKUP(annot)) {
    ...
    PopplerAnnotTextMarkup *text_markup =
        POPPLER_ANNOT_TEXT_MARKUP(poppler_annot);
    ...
    if (mask & EV_ANNOTATIONS_SAVE_TEXT_MARKUP_TYPE) {
      ...
      case EV_ANNOTATION_TEXT_MARKUP_HIGHLIGHT:
        new_annot = poppler_annot_text_markup_new_highlight(
            pdf_document->document, &rect, quads);
        break;
      case EV_ANNOTATION_TEXT_MARKUP_STRIKE_OUT:
        new_annot = poppler_annot_text_markup_new_strikeout(
            pdf_document->document, &rect, quads);
        break;
      case EV_ANNOTATION_TEXT_MARKUP_UNDERLINE:
        new_annot = poppler_annot_text_markup_new_underline(
            pdf_document->document, &rect, quads);
        break;
      case EV_ANNOTATION_TEXT_MARKUP_SQUIGGLY:
        new_annot = poppler_annot_text_markup_new_squiggly(
            pdf_document->document, &rect, quads);
        break;
      }
      ...
      g_object_set_data_full(G_OBJECT(annot), "poppler-annot", new_annot,
                             (GDestroyNotify)g_object_unref);
    }
...
pdf_document_attachments_get_attachments(EvDocumentAttachments *document) {
  ...
  for (list = attachments; list; list = list->next) {
    ...
    if (attachment_save_to_buffer(attachment, &data, &size, &error)) {
      ...
      ev_attachment = ev_attachment_new(
          attachment->name, attachment->description, mtime, ctime, size, data);
